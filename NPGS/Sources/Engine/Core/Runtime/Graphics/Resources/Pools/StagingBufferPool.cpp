...
namespace Npgs
{
    ...
    FStagingBufferPool::AcquireBuffer(vk::DeviceSize RequestedSize, const VmaAllocationCreateInfo* AllocationCreateInfo)
    {
        ...
        return AcquireResource(CreateInfo, [RequestedSize, AlignedSize, bAllocatedByVma](const FResourceInfo& BaseInfo) -> bool
        {
    ...
    void FStagingBufferPool::OptimizeResourceCount()
    {
        ...
        std::unordered_map<vk::DeviceSize, std::size_t> SizeUsageStats; // [Size, BufferUsage]
        std::unordered_map<vk::DeviceSize, std::size_t> SizeCountStats; // [Size, BufferCount]
        std::unordered_map<bool, std::size_t>           VmaUsageStats;  // [Vma,  BufferUsage]

        for (const auto& Resource : _AvailableResources)
        {
            auto& BufferInfo = static_cast<const FStagingBufferInfo&>(Resource);
            SizeUsageStats[BufferInfo.Size] = BufferInfo.UsageCount;
            ++SizeCountStats[BufferInfo.Size];
            VmaUsageStats[BufferInfo.Size] = BufferInfo.UsageCount;
        }
    }
